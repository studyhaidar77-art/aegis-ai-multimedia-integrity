...
# ✅ InsightFace cache folder (keeps it inside repo on cloud)
os.environ["INSIGHTFACE_HOME"] = "./.insightface"

# ✅ HF Spaces SAFE: Streamlit Columns.image() supports use_column_width (NOT use_container_width)
IMG_KW = {"use_column_width": True}
    ...
    if not frames:
        ...
        if all_faces_video:
            ...
            for i, face_img in enumerate(all_faces_video[:18]):
                cols[i % 6].image(face_img, caption=f"VFace {i+1}", **IMG_KW)
        else:
...
if suspect_photos:
    ...
    for i, f in enumerate(suspect_photos, start=1):
        ...
        cols[(i - 1) % 4].image(rgb, caption=f"Suspect Photo {i}", **IMG_KW)
    ...
    if all_faces_photo:
        ...
        for i, face_img in enumerate(all_faces_photo[:18]):
            cols[i % 6].image(face_img, caption=f"PFace {i+1}", **IMG_KW)
    else:
